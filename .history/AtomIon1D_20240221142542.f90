...
!******** CONSTRUCTION OF BASIS SETS ********
      ...
      call CalcHamiltonian(alpha,R(iR),mu,mi,theta_c,C4,L,Order,xPoints,&
         LegPoints,xLeg,wLeg,CB%xDim,xNumPoints,CB%u,CB%uxx,CB%xBounds,HalfBandWidth,CB%H,CB%S,P_Matrix)
...
subroutine CalcHSP(alpha,R,mu,mi,theta_c,C4,L,Order,xPoints,LegPoints,&
  ...
  do ix = 1,xDim
     do ixp = max(1,ix-Order),min(xDim,ix+Order)
        ...
        do kx = kxMin(ixp,ix),kxMax(ixp,ix)
           ...
           xV(ix,ixp) = xV(ix,ixp) + 0.5d0*mu*R*R*xVHO(ix,ixp)- R**(-4)*xVC4(ix,ixp)
        enddo
     enddo
  enddo
