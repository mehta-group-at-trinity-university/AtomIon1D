      ...
      Parameter(MaxChannels=5,MaxNumDataPts=145) !61
      Parameter(NumBound=5)
      Parameter(Rdelay=10.d0,Xdiv=5.)
      Parameter(NumFitTerms=3)
       ! This is for the atom-ion problem so use the Yb, Li masses
      integer maxdim,mrdim,NumDataPoints
      integer*2 ifa,ifb,mfa,mfb,onoff
      ...
      double precision Mass,massfactor
...
c  Only channels with NEGATIVE thresh(i) get their threshold energy fitted too
      ...
      open(100,file='fort.1000',status='old')
      open(200,file='fort.98',status='old')
      open(300,file='fort.99',status='old')

      massfactor = -0.5d0/Mass
      mrdim = MaxChannels

      do i=1,MaxNumDataPts
!       read(100,11) dummy,(dummyV(i,j),j=1,mrdim)
...
cc    %%%%  J=0+   %%%%
      ...
      Leff(NumBound+1) = 4.5d0;     L0(NumBound+1) = 1.d0;   
      Leff(NumBound+2) = 6.5d0;     L0(NumBound+2) = 1.d0;   
      Leff(NumBound+3) = 8.5d0;     L0(NumBound+3) = 3.d0;   
      Leff(NumBound+4) = 10.5d0;    L0(NumBound+4) = 1.d0;   
      Leff(NumBound+5) = 10.5d0;    L0(NumBound+5) = 3.d0;   

c     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

      write(19,*)'FitRangeMin =   OPTIMIZED'
...
c      Uu-(1/2mu)*Quu : (CC-CC)
     ...
     .               NumFitPoints,MaxNumDataPts,rmsError,FitCoeffs)
      ...
      if (FitRangeMinVar.ge.(FitRangeMax/Xdiv)) then
         ncount = ncount+1
         LeffMin(ncount) = dabs(Leff(j)-ang)
         Rmin(ncount) = FitRangeMinVar
         FitRangeMinVar = FitRangeMinVar-1.d0
         goto 123
      else
          FitRangeMinOpt(j) = Rmin(MinLoc(LeffMin,dim=1))
      endif
...
c       Quv : (BC-BC)        
        ...
        IF (thresh(j) .LT. 0.0d0 .AND. thresh(k) .LT. 0.0d0) THEN
           ...
           kExponents(3)=5.50d0
           ELSE
           kExponents(1)=3.50d0 ! the real coupling behaves like : exp(-R)
           kExponents(2)=4.50d0
           kExponents(3)=5.50d0
           ENDIF   
        ENDIF
...
c       Quv : (BC-CC)
        ...
        IF (thresh(j) .LT. 0.0d0 .AND. thresh(k) .GE. 0.0d0) THEN
           ...
           kExponents(1)=1.5d0+Leff(j)+1.d0+(1.d0)
           kExponents(2)=2.5d0+Leff(j)+1.d0+(1.d0)
           kExponents(3)=3.5d0+Leff(j)+1.d0+(1.d0)
           ELSE
...
c       Quv : (CC-CC)
        ...
        IF (thresh(j) .GE. 0.0d0 .AND. thresh(k) .GE. 0.0d0) THEN
           IF (Leff(j) .EQ. Leff(k) ) THEN  
           kExponents(1)=2.0d0+dabs(L0(j)-L0(k))+(1.d0)
           kExponents(2)=3.0d0+dabs(L0(j)-L0(k))+(1.d0)
           kExponents(3)=4.0d0+dabs(L0(j)-L0(k))+(1.d0)
           ELSE
           IF (L0(j) .EQ. L0(k) ) THEN   
           kExponents(1)=2.0d0+2.d0*max(L0(j),L0(k))+(1.d0)
           kExponents(2)=3.0d0+2.d0*max(L0(j),L0(k))+(1.d0)
           kExponents(3)=4.0d0+2.d0*max(L0(j),L0(k))+(1.d0)
           ELSE
           kExponents(1)=2.0d0+max(L0(j),L0(k))+(1.d0)
           kExponents(2)=3.0d0+max(L0(j),L0(k))+(1.d0)
           kExponents(3)=4.0d0+max(L0(j),L0(k))+(1.d0)
           ENDIF 
           ENDIF
        ENDIF
        ENDIF
...
c5152   format(3(1x,1pd14.7),5x,1pd10.3)
...
c       Puv : (BC-CC)
        IF (thresh(j) .LT. 0.0d0 .AND. thresh(k) .GE. 0.0d0) THEN
           IF (Leff(j) .GE. L0(k) ) THEN  
           kExponents(1)=1.5d0+Leff(j)+1.d0
           kExponents(2)=2.5d0+Leff(j)+1.d0
           kExponents(3)=3.5d0+Leff(j)+1.d0
           ELSE
...
c       Puv : (CC-CC)
        IF (thresh(j) .GE. 0.0d0 .AND. thresh(k) .GE. 0.0d0) THEN
           IF (Leff(j) .EQ. Leff(k) ) THEN  
           kExponents(1)=2.0d0+dabs(L0(j)-L0(k))
           kExponents(2)=3.0d0+dabs(L0(j)-L0(k))
           kExponents(3)=4.0d0+dabs(L0(j)-L0(k))
           ELSE
...
c3313 format(I4,1X,e16.8,1X,e16.8,1X,f16.8,1X,f16.8)
