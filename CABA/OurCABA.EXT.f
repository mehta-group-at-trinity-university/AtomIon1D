c234567890
      ...
      double precision rmsErrorMax,rmsErrorPMax,WhereStart,GridSwitch,BoxAvg,CrossTot
      ...
      integer NumClosedChannels,SwitchFlag
      ...
      character*64 LegendreFile,SFile,HFile,PFile,QFile,ParFile
      ...
      double precision Ri,Rf,Xi,Xf,Xmid,StepX,R(1:1000000)
...
c     read in Gauss-Legendre info
      ...
      read(5,*)
      read(5,*) LegPoints, GridSwitch, SwitchFlag
      read(5,*)
...
c     ––––––––––––––––––––––––––––––––––––––––––––––––––––––
c     Reading values from ParAndThresh.dat (L.O. 13 Dec 2024)
      open(18,file=ParFile,status='old')
      read(18,*)
      read(18,*) Ri, Rf, NumDataPoints
      
      if(Rf.lt.GridSwitch) then
        write(*,*)'GridSwitch larger than final R.'
        write(*,*)'Program ABORTED.'
        stop
      endif

      read(18,*) 
      read(18,*) NumTotStates

      if(NumTotStates.ne.NumChannels) then
c        write(*,*)'Check number of channels.'
c        write(*,*)'Program ABORTED.'
c        stop
        write(*,*)'NumTotStates and NumChannels are different.'
      endif

      read(18,*)
      do i=1,NumChannels
        read(18,*) dumb,Thresholds(i)
      enddo
      close(18)
c     ––––––––––––––––––––––––––––––––––––––––––––––––––––––
...
c     CUBIC RADIAL GRID BEFORE GridSwitch
      Xi = Ri**(1.0d0/3.0d0)
      Xmid = GridSwitch**(1.0d0/3.0d0)
      StepX = (Xmid-Xi)/dfloat(NPoints-1)
      ...
      if(SwitchFlag.eq.1) then
         NPointsW = int((Rf-GridSwitch)/rstep)
      elseif(SwitchFlag.eq.0) then
         NPointsW = 0
      endif
      ...
      if(NPointsW.gt.0) then
         do iR = 1,NPointsW
            R(iR+NPoints) = GridSwitch+(iR)*rstep
         enddo
      endif
c     an additional part of the radial grid is defined from
c     GridSwitch to Rf (the latter read from the input). 
c     ————————————————

      NumSectorsTot = NPoints+NPointsW
      write(6,*) "R(1)","  R(NumSectorsTot)","  NumSectorsTot","  NumPoints"
      write(6,*) R(1), R(NumSectorsTot), NumSectorsTot, NPoints 
...
c      XP = 1.d0/(NumBoxes-1)
c      NPS = int(XP*NumSectorsTot)
      NumSectorsBox(NumBoxes) = 2
      NPS = int((NumSectorsTot - NumRmatch)/(NumBoxes-1)) 
      do k = 1,NumBoxes-1
         NumSectorsBox(k) = NPS
      enddo 
      NumRmatch = NumSectorsTot-NPS*(NumBoxes-1)-NumSectorsBox(NumBoxes)  
c      Modified on December 16 by L.O. and N.P.M.
      ...
      write(17,1706)GridSwitch
      write(17,1707)NumRmatch
...
c      close(17)
 ...
 1706 format(1x,'GridSwitch =',f14.2)
 1707 format(1x,'NumRmatch =',I8)


c     %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c     HOW CAN I REMOVE ALL THIS PART?
...
c      open(18,file=FitFile,status='old') ! Contains fit parameters for VQ and for P?
c      read(18,*) NumTotStates,NumDataPoints
c      rmsErrorMax=0.d0
c      rmsErrorPMax=0.d0
c      do j=1,NumChannels
c         do k=j,NumChannels
c            read(18,*) jj,kk,(VQExponents(jj,kk,ii),ii=1,3)
c            read(18,*) (VQFit(jj,kk,ii),ii=1,3),rmsError
c            IF(j.NE.k) THEN
c               read(18,*) jj,kk,(PExponents(jj,kk,ii),ii=1,3)
c               read(18,*) (PFit(jj,kk,ii),ii=1,3),rmsErrorP
c            ENDIF
c            IF(jj.NE.j .OR. kk.NE.k) THEN
c               write(6,*) 'index mismatch!',j,jj,k,kk
c            ENDIF
c            DO i=1,3
c               ! enforce symmetry in VQ and antisymmetry in P
c               VQFit(k,j,i)=VQFit(j,k,i)
c               VQExponents(k,j,i)=VQExponents(j,k,i)
c               PFit(k,j,i)=-PFit(j,k,i)
c               PExponents(k,j,i)=PExponents(j,k,i)
c               IF(j.EQ.k) THEN
c                  PFit(k,k,i)=0.d0
c                  PExponents(k,k,i)=1
c               ENDIF
c            ENDDO
c            IF(rmsError .GT. rmsErrorMax) THEN
c               rmsErrorMax=dabs(rmsError)
c            ENDIF
c            IF(rmsErrorP.GT. rmsErrorPMax) THEN
c               rmsErrorPMax=dabs(rmsErrorP)
c            ENDIF
c         ENDDO
c         do k=Numchannels+1,NumTotStates
c            read(18,*)
c            read(18,*)
c            IF(j.NE.k) THEN
c               read(18,*)
     ...
     >     //trim(str(NumberR(NumBoxes)))//".dat")
      ...
      do ie=1,NumE
         ...
         do ibox = 1,NumBoxes-1
            ...
            if(ie.eq.1) write(17,*) "box number=",ibox
            do is=1,NumSectors+1
               xSector(is) = xSectorTot(ishift+is)
               if(ie.eq.1) write(17,*) ishift, is, ishift+is, xSectorTot(ishift+is)
            enddo
            Rstart = xSector(1)
...
c            NumSectors = ir
            ...
            do is=1,NumSectors+1
               xSector(is) = xSectorTot(ishift+is)
               if(ie.eq.1) write(17,*) ishift, is, ishift+is, xSector(is)
c               if(ir.eq.2) write(17,*) ishift, is, ishift+is, xSectorTot(ishift+is)
            enddo

            Rstart = xSector(1)
            Rmatch = xSector(NumSectors+1)

            if(ie.eq.1) write(17,*) "Rmatch loop:"
            if(ie.eq.1) write(17,*) "ir=",ir,"NumSectors=",NumSectors,"NumBoxes=",NumBoxes,"RMatch",RMatch
...
c     calculate S-matrix and scattering cross sections
     ...
     >           solution,deriv,Smatrix,Tmatrix,CrossSections,Kmatrix,Parity)
            ...
            do ic=1,ithresh-1 
c            do ic=1,ithresh
c           Tried to change this on 26 Nov 2024 (L.O.)            
...
c         write(27,20) energy, ((SmatrixAvg(i,j), i=1,NumOpenR), j=1,NumOpenR)
...
c     Write VQmat in another file to check indices (L.O. 28 Jan 2025)
c      open(unit = 105, file = "VQmat_check.dat")
c      do i = 1, NumDataPoints
c         write(105,11) xdata(i)
c         do j=1,Numchannels
c            write(105,11) (VQmat(i,j,k), k=1,Numchannels)
c         enddo
c      enddo
...
c                    vvj=VQFit(nc,jc,1)/x(j)**VQExponents(nc,jc,1)
c                    vvj=vvj+VQFit(nc,jc,2)/x(j)**VQExponents(nc,jc,2)
c                    vvj=vvj+VQFit(nc,jc,3)/x(j)**VQExponents(nc,jc,3)
c                    VQInterp(j)=vvj
c                    
c                    ppj=PFit(nc,jc,1)/x(j)**PExponents(nc,jc,1)
c                    ppj=ppj+PFit(nc,jc,2)/x(j)**PExponents(nc,jc,2)
c                    ppj=ppj+PFit(nc,jc,3)/x(j)**PExponents(nc,jc,3)
c                    PInterp(j)=ppj
...
c                vvj=VQFit(kjv,knv,1)/x(j)**VQExponents(kjv,knv,1)
c                vvj=vvj+VQFit(kjv,knv,2)/x(j)**VQExponents(kjv,knv,2)
c                vvj=vvj+VQFit(kjv,knv,3)/x(j)**VQExponents(kjv,knv,3)
c                VQInterp(j)=vvj
               
c                ppj=PFit(kjv,knv,1)/x(j)**PExponents(kjv,knv,1)
c                ppj=ppj+PFit(kjv,knv,2)/x(j)**PExponents(kjv,knv,2)
c                ppj=ppj+PFit(kjv,knv,3)/x(j)**PExponents(kjv,knv,3)
c                PInterp(j)=ppj
            enddo
