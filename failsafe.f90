...
program HHL1DHyperspherical
  ...
  double precision, allocatable :: nrg(:,:), left_nrg(:,:), right_nrg(:,:), tmp_nrg(:,:)
        ...
        call MyDsband(Select,CB%Energies,CB%Psi,MatrixDim,Shift,MatrixDim,CB%H,CB%S,HalfBandWidth+1,LUFac,LeadDim,HalfBandWidth,&
         ...
         if(iR .ne. RSteps) then
            ...
            do i=1, NumStates !populate after MyDSBand
               do j=2, RSteps-iR+1
                  curr_nrg(i,j) = tmp_nrg(i,j)
               enddo
               curr_nrg(i,1)=CB%energies(i,1) ! new energies in first column
            enddo
         call Avoided_Crossings(CB%Energies,oldEnergies, curr_nrg, (RSteps-iR), NumStates,(R(iR+1)-R(iR)), 1)!changes curr_nrg and CB%energies
            deallocate(tmp_nrg)
         ...
         if(iR .ne. RSteps) then
            allocate(tmp_nrg(NumStates, RSteps-iR))
            ! do i = 1, NumStates
            !    read(200,*, end = 100) (tmp_nrg(i,j), j = 1,(RSteps - iR)) ! Gets arrays for swapping; must update energies.dat after the swap
            ! enddo
            100 close(200)
            call Avoided_Crossings(CB%Energies,oldEnergies, tmp_nrg, (RSteps-iR), NumStates,(R(iR+1)-R(iR)), 1)            !do i = 1, NumStates
            
            write(200,20) (CB%Energies(i,1), i = 1,min(NumStates,iparam(5)))
            !   write(200, *) (tmp_nrg(i,j), j = 1, (RSteps - iR))
            !enddo
            deallocate(tmp_nrg)!!put this in the correct spot and add modification to the energy file 200
         else
            do i = 1, NumStates
               tmp_nrg(i,1) = CB%Energies(i,1)
            enddo
         endif
         allocate(NumStates, RSteps-iR+1)
...
integer state, ct, NumStates, tmp_size,i, swapstate
logical found_flag

found_flag = .FALSE.
state = NumStates-1
do while((found_flag.eqv..FALSE.).and.(state.ne.0))
