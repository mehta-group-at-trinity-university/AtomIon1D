...
program AtomIon1D
  ...
  integer NumStates,AsymptoticFlag,Order,Left,Right, RSteps, CrossingFlag
  double precision alpha,Shift,Shift2,NumStateInc,mi,ma,thetac,mgamma,mbeta,testR
  double precision RLeft,RRight,RDerivDelt,DD,L,RFirst,RLast,XFirst,XLast,StepX,xMin,xMax
  ...
  integer swapstate,iparam(11),ncv,info,countR
  integer i,j,k,m,n,iR,Nswap
  integer LeadDim,MatrixDim,HalfBandWidth
  integer xDim, Nbs, Ncol,NumOutputChannels, OPGRID
  ...
  read(5,*)
  read(5,*) NumStates,AsymptoticFlag
  write(6,*) NumStates,AsymptoticFlag
  ...
  mgamma = mu
  thetac = datan(mgamma)
  mbeta = sqrt(ma*mi)/(ma+mi)
  write(6,*) "thetac = ", thetac, "mbeta = ", mbeta
...
!   open(unit = 106, file = "QuickQtil.dat")
  ...
  !—————————————————————————————————
  ! Creates a file with limits of radial grid, number of
  ! output channels and thesholds for CABA. (L.O. 13 Dec 2024)
  !—————————————————————————————————
  open(unit = 305, file = "ParAndThresh.dat") 
  write(305,*) "Radial grid values:"
  write(305,*) RFirst, RLast, RSteps
  write(305,*) "Number of output channels:"
  write(305,*) NumOutPutChannels
  write(305,*) "Thresholds:"
  do i=1,NumOutPutChannels
     if(i.le.NumOutPutChannels-2*Nbs) then
       write(305,*) i, (i-1)+0.5d0
     else
       write(305,*) i, 1000+i*0.5d0
     endif
  enddo
  close(305)
  !—————————————————————————————————
  ...
  ! THIS IS NOT WORKING
    ...
    testR=sqrt(2d0*(abs(Eb(1)) &
      + AsymptoticU(mu,NumStates,24d0,NumStates,1,Eb,ZAvg,Z2Avg,thetac,mbeta))/(mu*cos(thetac)**2))+test_shift
  endif
  ...
  do iR = RSteps,1,-1
     ...
     if(R(iR).ge.testR) then  ! Conditional for using asymptotic forms
     ...
     else  ! conditional for switching to numerical calculations
        call CPU_TIME(t1)
        countR=countR+1
        if(countR.eq.1) then
          iRswitch = iR+1
        endif
        xMin = thetac + asin(dsqrt(mu/(1d0+mu**2))* sNb/R(iR)*(Rstar/lho)) !Based on fixed sNb
        xMax = Pi + thetac - asin(dsqrt(mu/(1d0+mu**2))* sNb/R(iR)*(Rstar/lho))
        
        sbc = (lho/Rstar)*R(iR)*dsqrt((1d0 + mu**2)/mu)*SIN(xMin - thetac)
        !print*, 'sbc using sbc = (lho/Rstar)*R(iR)*dsqrt((1+mu**2)/mu)*SIN(xMin - thetac):', sbc
        if(xMax.le.xMin) then
           write(6,*) "minimum hyperradius too small."
           stop
        endif
        call GridMakerIA(mu,mu12,thetac,Rstar/lho,R(iR),sbc,xNumPoints,xMin,xMax,xPoints,OPGRID)
        ...
        call CalcHSD(alpha,R(iR),mu,mi,thetac,C4,L,Order,xPoints,&
...
!        write(666,*)
        ...
        if(countR.eq.1) then ! Need to swap the lowest 2*Nbs states with the highest 2*Nbs states
           Perm = 0d0
           do i = 1,NumStates
              do j = 1, NumStates
                 !Perm(i,j)=kdelta(i,j)
                 if(i.le.NumStates-2*Nswap) then !Ncol = number of collision channels
                    Perm(i,j) = kdelta(i,j-2*Nswap)
                    if(Perm(i,j).ne.0d0) write(6,*) "collision channels  i,j = ", i,j
                 else
                    Perm(i,j) = kdelta(i,NumStates+1-j)
                    if(Perm(i,j).ne.0d0) write(6,*) "MI channels i,j =         ", i,j
                 endif
              enddo
           enddo
           ComposedPerm = Perm
        ...
        if(CountR.gt.1)  ComposedPerm = matmul(ComposedPerm,Perm)

        !call printmatrix(ComposedPerm,NumStates,NumStates,666)
        ...
        if(countR.gt.1) then
           ...
           do j = 1,NumStates
              ...
              if(testorth.lt.0d0) then ! Fix the phase of the diabatized wavefunctions
                 NewPsi(:,j) = -NewPsi(:,j)
        ...
        write(6,*) 'Remaining time (min): ', (t2-t1)*iR/60, R(iR), xMin, xMax, &
             (AllEnergies(i,iR), i=1,2), (AllEnergies(NumStates+1-i,iR), i=1,2*Nbs)
  ...
  do iR = iRswitch - ndelay + 1, Rsteps
      do m = 1,Numstates
         do n = m+1,NumStates
            if((n.gt.(NumStates-2*Nbs)).and.(m.gt.(NumStates-2*Nbs))) then
               P(n,m,iR) = testmat(n,m)*P(n,m,iR)
            elseif(iR.ge.iRswitch) then
               P(n,m,iR) = testmat(n,m)*P(n,m,iR)
            endif
            P(m,n,iR) = - P(n,m,iR)
         enddo
...
subroutine CalcCoupling_FH(NumStates,HalfBandWidth,MatrixDim,U,Psi,S,&
  ...
  double precision, external :: AsymptoticP
...
!
subroutine CalcHSD(alpha,R,mu,mi,thetac,C4,L,Order,xPoints,LegPoints,&
  ...
  double precision alpha,R,mu,mgamma,thetac,C4,L,mi
  double precision xPoints(*),xLeg(*),wLeg(*)
...
subroutine CalcHamiltonian(alpha,R,mu,mi,thetac,C4,L,Order,xPoints,LegPoints,&
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
subroutine GridMakerHHL(mu,mu12,mu123,thetac,R,r0,xNumPoints,xMin,xMax,xPoints)
  implicit none
  integer xNumPoints
  double precision mu,R,r0,xMin,xMax,xPoints(xNumPoints)
  double precision mu12,mu123,thetac
...
!ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
subroutine GridMakerIA(mu,mu12,thetac,Rstar,R,sbc,xNumPoints,xMin,xMax,xPoints,OPGRID)
  implicit none
  integer xNumPoints, testing
  double precision mu,R,r0,xMin,xMax,xPoints(xNumPoints)
  double precision mu12,mu123,thetac,Rstar,sbc
...
!!$c$$$      end
!!$c$$$
!!$c$$$      subroutine printVAIHO(thetac,mu,mi,ma)
!!$c$$$      implicit none
...
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
double precision function AsymptoticU(mu,ni,R,NC,Nbs,Eb,ZAvg,Z2Avg,thetac,mbeta)
  implicit none
  integer ni,n,NC,Nbs
  double precision R, mu, Eb(Nbs), thetac, mbeta, ZAvg(Nbs), Z2Avg(Nbs)
  !c  The asymptotic form of Un(R) 

  n=ni-1
  if(ni.le.(NC-2*Nbs)) then
     AsymptoticU = dble(n) + 0.5d0 - dble(1 + 2*n*(n+1))*0.25d0/(2d0*mu*R*R)! - 0.25d0/(2d0*mu*R*R)
  else if (ni.eq.(NC-2*Nbs+1)) then
     AsymptoticU = Eb(1) + 0.5d0*mu*R*R*cos(thetac)**2 &
          + 0.5d0*mu*sqrt(mbeta)*sin(2.0d0*thetac)*Zavg(1)*R !&
!          - 0.5d0*mu*mbeta*cos(2d0*thetac)*Z2Avg(1)
  else if (ni.eq.(NC-2*Nbs+2)) then
     AsymptoticU = Eb(1) + 0.5d0*mu*R*R*cos(thetac)**2 &
          - 0.5d0*mu*sqrt(mbeta)*sin(2.0d0*thetac)*Zavg(1)*R !&
!          - 0.5d0*mu*mbeta*cos(2d0*thetac)*Z2Avg(1)
  else if (ni.eq.(NC-2*Nbs+3)) then
     AsymptoticU = Eb(2) + 0.5d0*mu*R*R*cos(thetac)**2 &
          + 0.5d0*mu*sqrt(mbeta)*sin(2.0d0*thetac)*Zavg(2)*R !&
!          - 0.5d0*mu*mbeta*cos(2d0*thetac)*Z2Avg(2)
  else if (ni.eq.(NC-2*Nbs+4)) then
     AsymptoticU = Eb(2) + 0.5d0*mu*R*R*cos(thetac)**2 &
          - 0.5d0*mu*sqrt(mbeta)*sin(2.0d0*thetac)*Zavg(2)*R !&
!          - 0.5d0*mu*mbeta*cos(2d0*thetac)*Z2Avg(2)
  else if (ni.eq.(NC-2*Nbs+5)) then
     AsymptoticU = Eb(3) + 0.5d0*mu*R*R*cos(thetac)**2 &
          + 0.5d0*mu*sqrt(mbeta)*sin(2.0d0*thetac)*Zavg(3)*R !&
!          - 0.5d0*mu*mbeta*cos(2d0*thetac)*Z2Avg(3)
  else if (ni.eq.(NC-2*Nbs+6)) then
     AsymptoticU = Eb(3) + 0.5d0*mu*R*R*cos(thetac)**2  &
          - 0.5d0*mu*sqrt(mbeta)*sin(2.0d0*thetac)*Zavg(3)*R !&
!          - 0.5d0*mu*mbeta*cos(2d0*thetac)*Z2Avg(3)  
  else if (ni.eq.(NC-2*Nbs+7)) then                                 !
     AsymptoticU = Eb(4) + 0.5d0*mu*R*R*cos(thetac)**2 &            !
          + 0.5d0*mu*sqrt(mbeta)*sin(2.0d0*thetac)*Zavg(4)*R !&     !
!          - 0.5d0*mu*mbeta*cos(2d0*thetac)*Z2Avg(3)                ! Added by L.O.
  else if (ni.eq.(NC-2*Nbs+8)) then                                 ! to include the 4th bound state.
     AsymptoticU = Eb(4) + 0.5d0*mu*R*R*cos(thetac)**2  &           ! 
          - 0.5d0*mu*sqrt(mbeta)*sin(2.0d0*thetac)*Zavg(4)*R !&     ! (21 Jan 2025)
!          - 0.5d0*mu*mbeta*cos(2d0*thetac)*Z2Avg(3)                !
  endif
end function AsymptoticU
!cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
double precision function AsymptoticQtil(mu,mi,ni,R,NC,Nbs)
  implicit none
...
double precision function AsymptoticVQ(mu,mi,ni,R,NC,Nbs)
  implicit none
  integer mi,ni,m,n,NC,Nbs
  ...
  !c     Add the asymptotic form of Un(R)
  if(m.eq.n) then
     VQ = VQ + dble(n) + 0.5d0 - dble(1 + 2*n*(n+1))*0.25d0/(2d0*mu*R*R)
  endif
  AsymptoticVQ = VQ
!  write(6,*) "VQ = ",AsymptoticVQ
  ...
  if((mi.ne.ni).and.(mi.le.NC-2*Nbs).and.(ni.le.NC-2*Nbs)) then  ! For all couplings between collision channels
     ...
     if(abs(mi-ni).eq.2) then
        if((mi.eq.NC-2*Nbs+1).or.(ni.eq.NC-2*Nbs+1).or.(mi.eq.NC-2*Nbs+2).or.(ni.eq.NC-2*Nbs+2)) then
           ...
           ichoose = 3
           AsymptoticP = AsymPCoefBS(ichoose)/R
        else if((mi.eq.NC-2*Nbs+5).or.(ni.eq.NC-2*Nbs+5).or.(mi.eq.NC-2*Nbs+6).or.(ni.eq.NC-2*Nbs+6)) then  ! Added by L.O.
           ichoose = 6                                                                                      ! to include the 4th bound state.
           AsymptoticP = AsymPCoefBS(ichoose)/R                                                             ! (21 Jan 2025)
        endif
     else if(abs(mi-ni).eq.4) then
        if((mi.eq.NC-2*Nbs+1).or.(ni.eq.NC-2*Nbs+1).or.(mi.eq.NC-2*Nbs+2).or.(ni.eq.NC-2*Nbs+2)) then       ! Added by L.O.
           ichoose = 2                                                                                      ! to include the 4th bound state.
           AsymptoticP = AsymPCoefBS(ichoose)/R                                                             ! (21 Jan 2025)
        else if((mi.eq.NC-2*Nbs+3).or.(ni.eq.NC-2*Nbs+3).or.(mi.eq.NC-2*Nbs+4).or.(ni.eq.NC-2*Nbs+4)) then
           ichoose = 5
           AsymptoticP = AsymPCoefBS(ichoose)/R
        endif
     else if(abs(mi-ni).eq.6) then                                                                          !
        if((mi.eq.NC-2*Nbs+1).or.(ni.eq.NC-2*Nbs+1).or.(mi.eq.NC-2*Nbs+2).or.(ni.eq.NC-2*Nbs+2)) then       ! Added by L.O.
           ichoose = 4                                                                                      ! to include the 4th bound state.
           AsymptoticP = AsymPCoefBS(ichoose)/R                                                             !
        endif                                                                                               ! (21 Jan 2025)
     endif
  endif
