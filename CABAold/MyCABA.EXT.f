...
c      Energy = Egrid(1) !Einput + Thresholds(ithresh)
c       Energy = Egrid(1) + Thresholds(ithresh)
...
c     ————————————————
c     This makes WhereStart equal to the last point
c     on the radial grid taken from the AtomIon out
...
c     ————————————————
c     This makes Rf equal to WhereStar so that 
c     the fit is irrelevant. 
       if (Rf.ne.WhereStart) then 
           Rf = WhereStart
       endif
c     ————————————————
...
c     RADIAL GRID
      ...
      enddo
c     the radial grid is created from Ri (read from input file)
c     to the last value of R, which is taken from VQmat.dat
c     (note: WhereStart is set equal to this value at line 175)
c     the step is given by NPoints, which is in the input file.
...
c     WhereStart to Rf (the latter read from the input).
...
c     ————————————————
      NPointsW = 0
c     ————————————————
c     I am trying to avoid all this part because I think it's useless
c     if we do not care about the fit.    (L.O. 9 Dec 2024)
      ...
      enddo
c     this I don't understand what it means
