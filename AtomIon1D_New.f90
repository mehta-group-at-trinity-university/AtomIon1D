...
!   endif 
  ...
  CB%Right = Right
  RChange=100.d0
  write(200,*) "#", RSteps-1, NumOutputChannels
  ...
  write(305,*) RFirst, R(RSteps-1), RSteps-1
  ...
  !—————————————————————————————————————————————
  ! — MODIFIED WITH THE TEST ON ASYMPTOTICFLAG —
  ! — 29 JAN 2025 ——————————————————————————————
  !—————————————————————————————————————————————
  if(AsymptoticFlag.eq.1) then
  ...
  elseif(AsymptoticFlag.eq.0) then
      testR = RLast
  endif
