...
program HHL1DHyperspherical
  ...
  double precision, allocatable :: nrg(:,:), left_nrg(:,:), right_nrg(:,:)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   ! do i = 1, NumStates
   !    nrg(iR,i) = CB%Energies(i,1)
   !    slopes(iR,i) = (RB%Energies(i,1)-LB%Energies(i,1))/((R(iR)+RDerivDelt)-(R(iR)-RDerivDelt)) !Stores slope of each triad
   ! enddo
...
! at each R value (maybe at each R value greater than some point for efficiency), calculate the P/Q overlap between points in the triad
   ! if (close to 1) {do nothing}
   ! else {implement ~swap_crossings subroutine~}

! OR

! at each R value (maybe at each R value greater than some point for efficiency), calculate the P/Q overlap between points in the triad
   ! if (close to 1) {do nothing}
   ! else {store points that need to be swapped in buffer}
   ! swap_crossings
